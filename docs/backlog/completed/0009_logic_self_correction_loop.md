...
- [x] Logic correction loop exists and is bounded.
- [x] System differentiates runtime fixes vs logic fixes in UI/metadata.
- [x] Artifacts are persisted for transparency (what changed, why, and evidence).
...
- `backend/app/services/logic_validation_service.py` (new): Core validation with hybrid approach
- `backend/app/services/notebook_service.py` (modified): Added loop after execution success
- `tests/logic_validation/test_logic_validation_service.py` (new): 10 tests

Architecture:
1. After successful execution → LogicValidationService.validate()
2. Stage 1: Fast heuristic checks (no LLM cost)
3. Stage 2: LLM-as-judge for semantic validation
4. If FAIL: ask LLM to fix, re-execute (max 2 attempts)
5. Results stored in `cell.metadata.logic_validation`
